...
from .ops import MotionImage, plasma_fractal


class Fog:
    def __call__(self, img: Image.Image, mag=-1, prob=1.):
        ...
        max_val = img.max()
        fog = c[0] * plasma_fractal(wibble_decay=c[1])[:h, :w][..., np.newaxis]
        if is_gray:
            fog = np.squeeze(fog)
        else:
...
class Rain:
    def __call__(self, img, mag=-1, prob=1.):
        ...
        fillcolor = 200 if is_gray else (200, 200, 200)

        draw = ImageDraw.Draw(img)
        for _ in range(1, n_rains):
